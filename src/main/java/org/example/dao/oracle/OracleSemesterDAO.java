...
public class OracleSemesterDAO implements SemesterDAO {
    ...
    private static final String INSERT_SEMESTER = "Insert into SEMESTER values (SEMESTER_SEQ.nextval, ?, ?, ?, ?)";
    ...
    private static final String DELETE_MARKS_FOR_DELETING_SEMESTER = "delete from mark where lesson_id in " +
            "(select lesson_id from lesson where theme_id in " +
            "(select theme_id from theme where subject_details_id in" +
            "(select subject_details_id from subject_details where semester_id = ?)))";
    private static final String DELETE_LESSONS_FOR_DELETING_SEMESTER = "delete from lesson where theme_id in " +
            "(select theme_id from theme where subject_details_id in" +
            "(select subject_details_id from subject_details where semester_id = ?))";
    private static final String DELETE_THEMES_FOR_DELETING_SEMESTER = "delete from theme where subject_details_id in" +
            "(select subject_details_id from subject_details where semester_id = ?)";
    private static final String DELETE_SUBJECT_DETAILS_FOR_DELETING_SEMESTER = "delete from subject_details where semester_id = ?";
    private static final String GET_COUNT_OF_SEMESTERS = "select count(SEMESTER_ID) as AMOUNT from SEMESTER ";
    private static final String GET_SEMESTERS_BY_PAGE = "SELECT * FROM (SELECT p.*, ROWNUM rn FROM" +
            " (SELECT * FROM SEMESTER ORDER BY SEMESTER_ID) p) WHERE rn BETWEEN ? AND ?";
    ...
    public OracleSemesterDAO(ConnectionPool connectionPool, SchoolYearDAO schoolYearDAO) {
    ...
    public void deleteSemester(int id) {
        try (Connection connection = connectionPool.getConnection();
            ...
            try (PreparedStatement preparedStatement1 = connection.prepareStatement(DELETE_MARKS_FOR_DELETING_SEMESTER)) {
                LOGGER.info("Deleting marks for semester " + id + ".");
                preparedStatement1.setInt(1, id);
                preparedStatement1.executeUpdate();
            }
            try (PreparedStatement preparedStatement1 = connection.prepareStatement(DELETE_LESSONS_FOR_DELETING_SEMESTER)) {
                LOGGER.info("Deleting lessons for semester " + id + ".");
                preparedStatement1.setInt(1, id);
                preparedStatement1.executeUpdate();
            }
            try (PreparedStatement preparedStatement1 = connection.prepareStatement(DELETE_THEMES_FOR_DELETING_SEMESTER)) {
                LOGGER.info("Deleting themes for semester " + id + ".");
                preparedStatement1.setInt(1, id);
                preparedStatement1.executeUpdate();
            }
            try (PreparedStatement preparedStatement1 = connection.prepareStatement(DELETE_SUBJECT_DETAILS_FOR_DELETING_SEMESTER)) {
                LOGGER.info("Deleting subject details for semester " + id + ".");
                preparedStatement1.setInt(1, id);
                preparedStatement1.executeUpdate();
            }
